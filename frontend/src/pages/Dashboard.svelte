<script>
    ...
    function toggleItemState(item) {
        if (editMode) {
            ...
            document.getElementById(item.id).setAttribute("href", item.url);
        }
    }
...
<!--            Check if user has permissions (OIDC)-->
            <div class="inline-flex items-center space-x-2">
                ...
                <!--{{--                </svg>--}}-->
                <!--{{--            </a>--}}-->
                <button class="bg-gray-900 text-white/50 p-2 rounded-md hover:text-white smooth-hover" class:empty={'animate-bounce'}>
                ...
                {#if items}
                    <button on:click={toggleEditMode} class="bg-gray-900 text-white/50 p-2 rounded-md hover:text-white smooth-hover" class:editMode={'bg-gray-900/40 text-white/25'}>
                        <!--                @click="edit = !edit"-->
        ...
        {#if editMode}
            <div transition:blur={{ amount: 20, duration: 350 }} class="w-full bg-gray-700/40 rounded-lg p-4 text-center flex flex-col border-dashed border-4 border-gray-600">
